...
def Abort(message, doExit=1):
	...
	lm.util.CommandError"""
	
	syslog.syslog(syslog.LOG_ERR, message)
	raise lm.util.CommandError(message)
...
class DocStringHandler(handler.ContentHandler,
	...
	def getPlainText(self):
		if 'root' in self.users:
			...
			prompt = '$'
		s  = ''
		s += 'lm %s %s' % (self.name, self.getUsageText())
		s += '\n\nDescription:\n'
...
class Command:
	...
	def __init__(self, database):
		...
		self.text = ''
		self.server_config = lm.server_config
		self.server_site_config = lm.server_site_config
		self.version = lm.version
	...
	def abort(self, msg):
		...
		raise lm.utils.CommandError(msg)
			...
			if self.MustBeRoot and not \
				...
				try:
					...
					# DISABLE if self.newdb is not None:
						# DISABLE self.newdb.commit()
				except lm.utils.HostnotfoundException as e:
					if self.debug():
						traceback.print_exc()
					self.abort(str(e))
